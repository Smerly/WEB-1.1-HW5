...
def create():
    ...
    if request.method == 'POST':
        ...
        mongo.db.users.insert_one(new_plant)
        id = new_plant['name']
...
def detail(plant_id):
    ...
    #     'date_planted': request.form.get('date_planted')
    # }
    # plant_to_show = mongo.db.users.find_one(new_plant)
    plant_to_show = mongo.db.users.find_one({'name': plant_id})
    ...
    # collection.
    harvests = mongo.db.harvest.find({'name': plant_id})
...
def harvest(plant_id):
    ...
    # detail page form.
    # harvests = mongo.db.harvest.find(plant_id)
    ...
    mongo.db.users.insert_one(new_harvest)
...
def edit(plant_id):
    ...
    if request.method == 'POST':
        ...
        changeParam = {'$set': {
            ...
            'photo_url': request.form.get('photo'),
            'date_planted': request.form.get('date_planted')}}
        mongo.db.users.update_one(searchParam, changeParam)
        return redirect(url_for('detail', plant_id=plant_id))
    else:
        # TODO: Make a `find_one` database call to get the plant object with the
        # passed-in _id.
        mongo.db.users.find_one({'name': plant_id})
        plant_to_show = ''
...
def delete(plant_id):
    ...
    mongo.db.users.delete_one({'name': plant_id})
    # TODO: Also, make a `delete_many` database call to delete all harvests with
    # the given plant id.

    mongo.db.users.delete_many({'name': plant_id})
